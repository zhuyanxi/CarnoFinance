...
const (
	...
	outputFileName  = "csi1000_close_move_10pct_natural_days.csv"
	summaryFileName = "csi1000_close_move_10pct_summary.md"
	bucketFileName  = "csi1000_close_move_10pct_bucket_distribution.csv"
...
func main() {
	...
	results := computeFirst10PctMoveDays(series)
...
func readDailyClose(path string) ([]dailyClose, error) {
	...
	for {
		...
		series = append(series, dailyClose{
			TradeDate: tradeDate,
			Close:     closePrice,
		})
	}
...
func computeFirst10PctMoveDays(series []dailyClose) []moveResult {
...
func writeResults(path string, series []dailyClose, results []moveResult) error {
	...
	if err := writer.Write([]string{"trade_date", "natural_days_to_10pct_close_move", "breach_date", "breach_close"}); err != nil {
		return err
	}
